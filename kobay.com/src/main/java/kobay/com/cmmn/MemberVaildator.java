...
public class MemberVaildator implements Validator {
	...
	public void validate(Object obj, Errors errors) {
		...
		if(!check) {
			errors.rejectValue("member_id","email","유효한 이메일이 아닙니다.");
		...
		if(!check || mPwd.length() < 8) {
			errors.rejectValue("member_pwd","pwd","유효한 비밀번호가 아닙니다.");
		...
		if(!check1 && !check2) {
			errors.rejectValue("member_name","name","유효한 이름이 아닙니다.");
		...
		if(!check) {
			errors.rejectValue("member_phone","phone","유효한 핸드폰 번호가 아닙니다.");
